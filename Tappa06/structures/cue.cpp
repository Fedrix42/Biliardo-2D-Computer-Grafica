...
std::optional<Ball> Cue::advance(sf::Time current_t)
{
    ...
    */
    if(shotting){
        ...
        if(current_t - shot_start_animation >= sf::seconds(1)){
            if(backward){ // Ora devo andare avanti
                backward = false;
                shot_start_animation = current_t;
            } else {
                // Animazione terminata
                Ball materialized = Ball(99, tip.getRadius(), 0, tip.getPosition());
...
{
    if(type == CueType::FREE){
        sf::Angle angle = (clockwise) ? sf::degrees(5) : sf::degrees(-5);
