...
namespace utils {
    namespace physics {
        ...
        sf::Vector2f speedAfterTime(sf::Vector2f speed, float deceleration, sf::Time delta){
            float speed_module = vectors_screen_space::norm(speed);
    ...
    namespace monocollisions {
        ...
        */
        float finalSpeed1D_ofA(float massA, float massB, float inspeedA, float inspeedB){
            ...
            float comp2 = ((2.0f*massB) / msum) * inspeedB;
            /*
            std::cout << "fnspeedA_1D(" << massA << "," << massB << "," << inspeedA << "," << inspeedB << ")"
                    << "\n\t msum: " << msum
                    << "\n\t comp1: " << comp1
                    << "\n\t comp2: " << comp2
                    << "\n\t final: " << comp1 + comp2 << std::endl;
            */
            return comp1 + comp2;
        }
    }
