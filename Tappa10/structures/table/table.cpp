...
Table::Table(sf::Vector2f table_size, sf::Vector2f offset)
 : cue(table_size / 2.0f + offset){
    ...
    std::cout << "DEBUG Creating White ball based on table position: " 
        << utils::point_to_str(shape.getPosition()) << ", "
        << utils::point_to_str(shape.getSize()) << std::endl;
...
void Table::reset_white(){
    // std::cout << "DEBUG Resetting White ball " << BallIDRange::WHITE << std::endl;
    ...
    std::cout << "DEBUG Resetting White ball based on table position: " 
        << utils::point_to_str(shape.getPosition()) << ", "
        << utils::point_to_str(shape.getSize()) << " - result: "
        << utils::point_to_str(white.get_position()) << std::endl;
    white.set_speed({0, 0});
}
