    ...
    */
    if(black_in_pocket){
        if(current->remaining > 0 || white_in_pocket){
            if(white_in_pocket){
                panel->setMiddle(current->name + ", you LOST! You put the ball 8 in the pocket but also the white one...");
            } else {
                panel->setMiddle(current->name + ", you LOST! You put the ball 8 in the pocket with remaining balls...");
            }
        } else {
            panel->setMiddle(current->name + ", you WON!");
        }
        return true;
    }
