...
void GameState::update(sf::Time current_t)
{
    ...
    // Lo faccio solo ogni tot ms perchè la compute collisions genera collisioni nel futuro
    // quindi devo dare alle palline il tempo di catturare quelle collisioni future
    if(simulation && (current_t - ss.shot_time).asMilliseconds() > 500){
        simulation = false;
    ...
    if(!simulation){
        GameLogic::round(&ss, &table, current, other, &panel, &config); // Valuta il risultato del round
    }
